...
impl<'a, P: Param> ParamSlider<'a, P> {
    ...
    fn begin_keyboard_entry(&self, ui: &Ui) {
        ui.memory_mut(|i| i.request_focus(self.keyboard_focus_id.unwrap()));
    ...
    fn keyboard_entry_active(&self, ui: &Ui) -> bool {
        ui.memory(|i| i.has_focus(self.keyboard_focus_id.unwrap()))
    }
    ...
    fn get_drag_amount_memory(ui: &Ui) -> f32 {
        ui.memory(|i| i.data.get_temp(*DRAG_AMOUNT_MEMORY_ID).unwrap_or(0.0))
    }

    fn set_drag_amount_memory(ui: &Ui, amount: f32) {
        ui.memory_mut(|i| i.data.insert_temp(*DRAG_AMOUNT_MEMORY_ID, amount));
    }
